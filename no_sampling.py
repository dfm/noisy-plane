...
from no_sampling_like import lnlike, period_model
import h5py
...
def lnprob(m, age_obs, bv_obs, period_obs, period_err, logg_obs, c):
    lp = lnprior(m)
    if not np.isfinite(lp):
        return -np.inf
    return lp + lnlike(m, age_obs, bv_obs, period_obs, period_err, logg_obs, c)
...
def MCMC(fname, c):
    ...
    args = (age_obs, bv_obs, period_obs, period_err, logg_obs, c)
