...
def log_period_model(par, log_age, temp):
    T_k = 6250
    return par[0] + par[1] * log_age + par[2] * np.log10(T_k - temp)
...
par_true = [np.log10(0.7725), 0.5189, .2]
...
par_true = [np.log10(0.7725), 0.5189, .2, 6250]
# plot period vs age
pl.clf()
...
mcmc_result = np.concatenate((mcmc_result, np.array([6250]))
plots = pretty5.plotting()
